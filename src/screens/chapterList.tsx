...
export function ChapterList() {
  ...
  const [renderedChapterList, setRenderedChapterList] = useState<
    ChapterContent[]
  >([]);
          ...
          setRenderedChapterList([
            {
              name:
                "Continue at: " +
                chapters[selectedNovel.lastPosition.chapterIndex].name,
              chapterIndex: -1,
              chapterPath: "",
            },
            ...chapters,
    ...
    <AppShell header={<></>}>
      <div className="px-3 pt-4 pb-8 space-y-3">
        ...
        <Page className="flex flex-col gap-y-4 mt-4">
          ...
          {loadedChaptersState &&
            renderedChapterList.map((chapterContent, index) => {
              return (
                <Card variant="elevated" className="flex items-center gap-x-2">
                  <button
                    className="flex-1 min-w-0"
                    onClick={() => {
                      selectChapter(chapterContent.chapterIndex);
                    }}
                  >
                    <ListItem
                      leading={
                        chapterContent.chapterIndex != -1 ? (
                          <h2>{chapterContent.chapterIndex}</h2>
                        ) : (
                          ""
                        )
                      }
