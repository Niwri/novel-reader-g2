...
const END_PADDING = "\n";
const CHAPTER_MENU_ITEM_COUNT = 4;
...
function getChapterParts(snapshot: AppSnapshot) {
  const title =
    "── " + truncateLabel((snapshot.title ?? "").trim(), 50) + " ──";
...
function findLineIndexAtOrBeforeOffset(
  lineStarts: number[],
  offset: number,
): number {
...
function buildWindowInfo(
  ...
  const maxStartLineIndex = findLineIndexAtOrBeforeOffset(
    lineStarts,
    rawMaxOffset,
  );

  // GET NEXT CHAPTER OFFSET
  const refNextOffset = Math.max(
    0,
    paddedBody.length - maxLength + END_PADDING.length,
  );
  const refMaxStartLineIndex = findLineIndexAtOrBeforeOffset(
    lineStarts,
    refNextOffset,
  );
  ...
  const startLineIndex = Math.max(
    0,
    Math.min(requestedLineIndex, maxStartLineIndex),
  );
...
function shiftWindowOffset(
  ...
  const stepLines = Math.max(
    1,
    (current.lineCount || 1) - WINDOW_OVERLAP_LINES,
  );

  if (direction === "down") {
    const nextStartLine = Math.min(
      maxStartLineIndex,
      current.startLineIndex + stepLines,
    );
...
function updateChapterScroll(
  ...
  const current = buildWindowInfo(
    body,
    scrollState.chapterScrollOffset ?? 0,
    availableBodyChars,
  );
  ...
  if (direction === "up") {
    ...
    const nextOffset = shiftWindowOffset(
      body,
      current,
      "up",
      availableBodyChars,
...
export const chapterScreen: any = {
  ...
  action(action: any, nav: any, snapshot: AppSnapshot, ctx: AppActions) {
    if (action.type === "HIGHLIGHT_MOVE") {
      // Triggers when scrolling at the bottom or top of the text container!
      if (nav.toggleMenu) {
        const maxHighlightIndex = CHAPTER_MENU_ITEM_COUNT - 1;
...
export function buildChapterRebuildContainer(
  snapshot: AppSnapshot,
  nav: any,
  containerID = 3,
) {
  const { body, availableBodyChars } = getChapterParts(snapshot);
  const scrollState = getScrollState(nav);
  const windowedBody = buildWindowInfo(
    body,
    scrollState.chapterScrollOffset ?? 0,
    availableBodyChars,
  ).content;

  const titleText = truncateLabel(
    "Chapter: " + (snapshot.title?.trim() ?? ""),
    45,
  );

  const menu = new ListContainerProperty({
    xPosition: DISPLAY_W / 2 - 9.5 * 10,
  ...
  const title = new TextContainerProperty({
    xPosition: DISPLAY_W / 2 - 4 * (titleText.length ?? 0),
    yPosition: DISPLAY_H / 2 - 20 * 4 - 30,
    width: 10 * (titleText.length ?? 0),
    height: 30,
    borderColor: 0,
    borderRadius: 0,
    borderWidth: 0,
    containerID: containerID + 2,
    containerName: "menuchaptername",
    content: titleText ?? "",
    isEventCapture: 0,
  });
  ...
  // Keep a valid (non-empty) fallback list container so page structure remains stable.
  const dummy = new ListContainerProperty({
    xPosition: -1000,
    yPosition: -1000,
    width: 1,
    height: 1,
  ...
  return new RebuildPageContainer({
    containerTotalNum: 2,
    listObject: scrollState.toggleMenu ? [menu] : [dummy],
    textObject: scrollState.toggleMenu ? [title] : [text],
    imageObject: [],
  });
}
